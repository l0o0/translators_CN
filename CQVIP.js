...
function detectWeb(doc, _) {
	let panel = doc.querySelector('#articlelist, .j-d-articles');
	if (panel) {
...
function getSearchResults(doc, checkOnly) {
	...
	var rows = Array.from(doc.querySelectorAll('#articlelist dl, .j-d-articles li')).filter(row => row.querySelector('dt:first-child > a, a'));
	for (let row of rows) {
		let doi = row.querySelector('.doi > i > a');
		let href = row.querySelector('dt:first-child > a, a').href;
		let title = attr(row, 'dt:first-child > a, a', 'title') || ZU.trimInternal(row.querySelector('dt:first-child > a, a', 'title').textContent);
...
async function doWeb(doc, url) {
	if (detectWeb(doc, url) == 'multiple') {
		...
		if (!items) return;
		for (let key of Object.keys(items).map(aKey => JSON.parse(aKey))) {
			// 需要浏览器环境
			let newItem = await scrape(await requestDocument(key.url));
...
async function scrape(doc, url = doc.location.href) {
	...
	switch (newItem.itemType) {
		case 'journalArticle': {
			...
			let pubInfo = labels.getWith('年卷期');
			...
			creators = Array.from(doc.querySelectorAll('.author > a')).map(element => processName(ZU.trimInternal(element.textContent)));
		...
		case 'thesis':
			...
			creators = Array.from(doc.querySelectorAll('.author > a')).map(element => processName(ZU.trimInternal(element.textContent)));
			labels.getWith('导师姓名').split(/[;，；]/).forEach(creator => creators.push(processName(creator, 'contributor')));
			break;
		...
		case 'patent':
			...
			creators = Array.from(labels.getWith('发明人', true).querySelectorAll('a')).map(element => processName(ZU.trimInternal(element.textContent)));
			break;
		...
		*/
		case 'statute': {
			...
			extra.set('applyDate', labels.getWith('实施日期'));
			creators = Array.from(labels.getWith('颁布部门', true).querySelectorAll('a')).map(element => processName(ZU.trimInternal(element.textContent)));
			break;
		}
		case 'report':
...
var testCases = [
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/2031156636722",
		"items": [
			{
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20157997658",
		"items": [
			{
				"itemType": "book",
				"title": "电视色彩学",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20471967745",
		"items": [
			{
				"itemType": "thesis",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20668882544",
		"items": [
			{
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20512556185",
		"items": [
			{
				"itemType": "standard",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/207336700438",
		"items": [
			{
				"itemType": "patent",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20843700223",
		"items": [
			{
				"itemType": "statute",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20865128867",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20858657837",
		"detectedItemType": "statute",
		"items": [
			{
				"itemType": "report",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/20913559137",
		"items": [
			{
				"itemType": "report",
				"title": "光伏储能一体机系统",
	...
	{
		"type": "web",
		"url": "http://k.cqvip.com/asset/detail/21136403096",
		"items": [
			{
				...
				"date": "2021-12-21",
				"abstractNote": "文化是城市发展的基因和血脉，旅游是城市发展的窗口和名片。省第十二次党代会旗帜鲜明地提出实施“强省会战略，长沙如何发挥龙头引领和辐射带动作用，以省会文化旅游的高质量牵引全省发展？“坚持文旅牵引，推动文旅率先，奋力在‘强省会’战略中当先锋、打头阵、显作?",
				"language": "zh-CN",
				"libraryCatalog": "维普经纶知识服务平台",
				"pages": "4",
				"publicationTitle": "长沙晚报",
				"url": "http://k.cqvip.com/asset/detail/21136403096",
				"attachments": [],
				"tags": [
					{
