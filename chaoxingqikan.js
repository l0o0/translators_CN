{
	...
	"lastUpdated": "2023-10-24 11:29:38"
}
...
function getPureText(element) {
	...
	while (elementCopy.firstElementChild) {
...
function matchSource(element) {
	// Z.debug(element);
	let source = {};
	source.publicationTitle = element.querySelector('a.jourName').innerText.replace(/[《》]/g, '');
...
async function scrape(doc, url = doc.location.href) {
	...
	newItem.title = doc.querySelector('h1[class^="F_titel"]').innerText;
	newItem.abstractNote = doc.querySelector('#generateCatalog_0').innerText;
	newItem.creators = getPureText(doc.querySelector('p.F_name'))
		.split(/[,，]/)
	...
	newItem.tags = getPureText(doc.querySelector('#generateCatalog_1')).split(' ').map(tag => ({ tag: tag }));
	...
	var journalURL = doc.querySelector('div[class^="sTopImg"] > p:first-of-type > a').href;
	let parser = new DOMParser();
	let journalDoc = parser.parseFromString((await requestText(journalURL)), 'text/html');
	// Z.debug(journalDoc);
	var journalInfo = {};
	ZU.xpath(journalDoc, '//div[@class="header-detail"][1]/div').forEach((element) => {
	...
	let pdfURL = doc.querySelector('a[class*="pdf-down"]').href;
	// Z.debug(pdfURL);
