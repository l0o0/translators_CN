...
async function scrape(doc, url = doc.location.href) {
	Z.debug("--------------- WanFang Med 2023-12-26 14:49:28 ---------------");
	let ids = new ID(doc, url);
	...
	newItem.extra += addExtra('original-title', text(doc, '.headline > h3'));
	newItem.extra += addExtra('cite', tryMatch(text(doc, '.statistics .ico-link'), /\d+/));
	switch (newItem.itemType) {
		case 'journalArticle': {
			let pubInfo = labels.getWith('期刊');
			newItem.publicationTitle = publicationTitle;
			newItem.volume = tryMatch(pubInfo, /0*(\d+)卷/, 1);
			newItem.issue = tryMatch(pubInfo, /0*(\d+)期/, 1);
		...
		case 'patent': {
			...
			newItem.place = labels.getWith('国别省市代码');
			newItem.country = newItem.place;
			newItem.assignee = labels.getWith('申请/专利权人');
			newItem.patentNumber = labels.getWith('申请/专利号');
		...
		case 'standard':
			...
			addExtra('apply date', labels.getWith('实施日期'));
			break;
		case 'statute':
